...
public:
    ...
    void AddEpollEvent(std::shared_ptr<Channel> event_channel);   //向事件池以及内核事件表中添加新的事件
    void ModEpollEvent(std::shared_ptr<Channel> event_channel);   //修改内核事件表中注册的文件描述符想要监听的事件
    void DelEpollEvent(std::shared_ptr<Channel> event_channel);   //删除内核事件表中注册的文件描述符

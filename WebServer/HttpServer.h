...
public:
    friend HttpServer& CreateMainReactor(int port);
    HttpServer(const HttpServer&) = delete;
    HttpServer& operator=(const HttpServer&) = delete;

    void Start();
private:
    explicit HttpServer(int port);                              //私有构造函数以限制对象的数量
...
inline
HttpServer& CreateMainReactor(int port)
{
    static HttpServer main_reactor(port);
    return main_reactor;
}
